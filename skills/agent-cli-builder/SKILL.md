...
| **[`ant`](https://github.com/anthropics/anthropic-cli)** (Claude Developer Platform) | API CLI generated from OpenAPI spec | Stainless-generated from the spec — same schema → CLI + SDK; 7-format output (`auto`, `json`, `jsonl`, `pretty`, `raw`, `yaml`, `explore`) with separate `--format-error`; built-in `--transform` / `--transform-error` via GJSON (eliminates `| jq` piping); `@file://` / `@data://` inline file substitution anywhere in nested payloads; relaxed YAML-flavored JSON input (no quoting keys); 5-tier credential precedence (flag→env→explicit-profile→federation→implicit-profile) with one-shot multi-source warning; full profile system (`ant profile activate/list/get/set`); careful pipe detection handling Cursor / CI false-positive edge cases. |
