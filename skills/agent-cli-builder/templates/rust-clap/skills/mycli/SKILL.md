...
# 4. List recent tasks:
mycli task list --state running --output json
```
...
- `--dry-run` returns a structured plan in `data.would_emit` / `metadata.would_emit`; parse that field to see the planned action.
...
- Mistyped commands (`mycli helo`) suggest the closest valid command via clap's built-in suggestion engine; read the suggestion before retrying.
