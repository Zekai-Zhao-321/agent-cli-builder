...
7. **"Does my CLI live in an ecosystem with established conventions? Am I matching the vocabulary the agent already has muscle memory for?"** Agents build a generalized model from every CLI they've ever seen. `gh`, `kubectl`, `aws`, `wrangler` all use roughly the same verbs and flag forms. If your CLI uses `info` where everyone else uses `get`, or `--skip-confirmations` where everyone else uses `--force`, the agent doesn't fail outright — it succeeds slowly, after a `--help` round-trip, after burning tokens on disambiguation. Match the ecosystem; deviate only with strong reason.
